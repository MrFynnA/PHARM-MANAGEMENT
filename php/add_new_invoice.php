...
if (isset($_GET['action']) && $_GET['action'] == "is_customer")
  isCustomer(strtoupper($_GET['name']), $_GET['contact_number'], $_GET['contact_address']);
...
function isCustomer($name, $contact_number, $contact_address = '')
{
  ...
  if ($con) {
    ...
    if ($result->num_rows == 0) {
      $addCus = "INSERT INTO customers (NAME,CONTACT_NUMBER, ADDRESS, DOCTOR_NAME, DOCTOR_ADDRESS) VALUES('{$name}','{$contact_number}', '{$contact_address}', '', '')";
      $resp = mysqli_query($con, $addCus);
...
function getCustomerId($name, $contact_number)
{
  ...
  if ($con) {
    ...
    $result = mysqli_query($con, $query)->fetch_array();
    // var_dump($result);exit;
    // $data = mysqli_fetch_array($result);
    $row = json_decode(json_encode($result, true), true);
    return (count($row) > 0) ? $row['ID'] : 0;
  }
}
